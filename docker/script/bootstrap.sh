...
# This shell script sets up a Docker image environment for Apache Airflow. It installs system dependencies,
# sets up a user, installs Python 3.11, upgrades pip, installs Java, handles MariaDB dependencies,
# installs and configures Airflow with specific extras (like Celery and StatsD), and installs various tools and
# packages (libxml2, libxslt, libcurl, pycurl, postgresql, unixODBC-devel, jq, nc, zip, unzip, bzip2, gzip, awscli).
# Finally, it cleans up the package manager caches.
...
BASE_PROVIDERS_FILE=/base-providers-requirements.txt
echo "Installing providers supported for airflow version ${AIRFLOW_VERSION}"
sudo -u airflow pip3 install --constraint /constraints.txt $PIP_OPTION -r $BASE_PROVIDERS_FILE
